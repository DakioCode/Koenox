...
namespace Koenox\API\InventoryUI;
...
use Koenox\API\InventoryUI\exception\IllegalInventorySizeException;
...
class CustomInventory extends SimpleInventory
{
     ...
     */
    protected function setDefaultItems(array $items): void
    {
    ...
    protected function isDefaultItem(Item $item): bool
    {
        foreach ($this->defaultItems as $defaultItem) {
            if ($defaultItem->getVanillaName() === $item->getVanillaName())
                return true;
        }
        return false;
    }
